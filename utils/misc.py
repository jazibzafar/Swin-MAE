...
class MetricLogger(object):
    def __init__(self, len_iterable, delimiter="\t"):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.len_iterable = len_iterable
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        space_fmt = ':' + str(len(str(self.len_iterable))) + 'd'
        log_msg = [
            header,
            '[{0' + space_fmt + '}/{1}]',
            'eta: {eta}',
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == self.len_iterable - 1:
                eta_seconds = iter_time.global_avg * (self.len_iterable - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    print(log_msg.format(
                        i, self.len_iterable, eta=eta_string,
                        meters=str(self),
        ...
        print('{} Total time: {} ({:.4f} s / it)'.format(
            header, total_time_str, total_time / self.len_iterable))
